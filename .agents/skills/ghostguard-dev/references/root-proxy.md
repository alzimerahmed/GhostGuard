...
GhostGuard supports a Root Proxy mode as an alternative to the standard `VpnService`. This mode requires a rooted device (e.g., via Magisk or KernelSU) and utilizes `libsu` and `iptables` to redirect DNS traffic transparently.
