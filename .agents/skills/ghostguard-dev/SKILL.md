---
name: ghostguard-dev
description: Assistance with Go and Kotlin interop, VPN architecture, and tunnel building in the GhostGuard Android app. Use when adding features to the Go tunnel, working with gomobile, or modifying the VPN service.
---

# GhostGuard Dev Skill

## Overview

This skill provides expert guidance on the `GhostGuard` project, which combines a Go-based VPN tunnel (`tunnel/`) with an Android Kotlin application (`app/src/main/java/...`). It helps handle `gomobile` interop constraints, architectural patterns for the VPN service, and building/debugging the tunnel.
