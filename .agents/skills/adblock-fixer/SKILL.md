...
Skill chuyên dụng để chẩn đoán, loại bỏ quảng cáo, vượt qua các tường lửa Anti-AdBlock (AdBlock Detected modal, lock scroll), và chặn quảng cáo dạng video/popunder trong trình duyệt In-App Browser của GhostGuard Android.
...
1. Tìm PID và chuyển tiếp cổng WebView DevTools:
   ```bash
   APP_PID=$(adb shell pidof app.ghostguard)
...
- **Tập tin**:
  - `app/src/main/java/app/ghostguard/ui/browser/rules/BrowserRuleDefaults.kt`
  - `rules/browser_rules.json`
  - `app/src/main/java/app/ghostguard/ui/browser/interceptor/BrowserAdBlocker.kt`
- **Quy tắc**:
